const getTodoToPreview=()=>{
    ...
    const {date,title,id}=previewTodo
    const mappedTodos = 

        ` 
        <div class="flex justify-between items-center justify-center">
      <div>
            <b class="font-bold text-slate-900 container text-2xl">${title}</b><div class="text-[9px] text-gray-700">${todoDate(previewTodo.created_on)}</div>
          </div>
      
      <section class="flex gap-6">
        <button onclick="handleEditMode('${id}')" title="Edit">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="black" class="w-6 h-6">
        <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />   
        </svg>
        </button>
          
        <button class='' onclick="deleteTodo('${id}')" title="Delete">
      ...
      <div class=" flex items-center justify-between border border-b-slate-300 mt-4 p-2 group">
        <div><b class="text-gray-400">Description:</b><br>
         <code>here is a description for your Todo 
         </code></div>


         <button class="hidden group-hover:block" onclick="handleEditMode('${uuid}')" title="Edit">
